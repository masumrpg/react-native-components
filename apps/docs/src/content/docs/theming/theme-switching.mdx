...
<CardGrid>
  ...
  <Card title="🎨 Preset Switching" icon="palette">
  ...
  return (
    <Animated.View style={[styles.fab, animatedStyle]}>
      <TouchableOpacity onPress={toggleTheme} style={styles.fabButton}>
        <Text style={styles.fabIcon}>{isDark ? '☀️' : '🌙'}</Text>
      </TouchableOpacity>
    </Animated.View>
  );
}
...
<CardGrid>
  <Card title="🎯 User Control" icon="star">
  ...
  <Card title="✨ Smooth Transitions" icon="setting">
...
<Tabs>
  ...
  <TabItem label="❌ Don'ts">
    ```tsx
    // ❌ Don't switch themes without user action
    useEffect(() => {
    ...
    // ❌ Don't make switching hard to find
...
<CardGrid>
  ...
  <Card title="Performance Tips" icon="rocket">
    ...
    [Setup Guide →](/getting-started/setup-guide)
  </Card>
</CardGrid>